 ...
 */
class DevicewithOtherStatesFactory extends Factory
{
    ...
    public function definition()
    {
        if ($this->faker->boolean(50)) {
            $inv = 'MOB-' . $this->faker->unique()->numberBetween(1000, 9999);
            $typeInitials = 'MOB';
        } else {
            $inv = 'TAB-' . $this->faker->unique()->numberBetween(1000, 9999);
            $typeInitials = 'TAB';
        }
        ...
        $availableStates = Status::whereNot('status_name','in Store')
                                    ->whereNot('status_name','assigned')
                                    ->whereNot('status_name','stationary')
                                    ->whereNot('status_name','offboarding')
                                    ->pluck('id');
        ...
        return [
            ...
            'status_id' => $this->faker->randomElement($availableStates),
            'user_id' => 1,
